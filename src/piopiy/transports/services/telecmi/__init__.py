...
class TelecmiInputTransport(BaseInputTransport):
    ...
    async def _audio_in_task_handler(self):
        ...
        async for audio_data in audio_iterator:
            if audio_data:
                ...
                piopiy_audio_frame = await self._convert_telecmi_audio_to_piopiy(
                    audio_frame_event
                )
    ...
    async def _convert_telecmi_audio_to_piopiy(
...
class TelecmiOutputTransport(BaseOutputTransport):
    ...
    async def write_audio_frame(self, frame: OutputAudioRawFrame):
        telecmi_audio = self._convert_piopiy_audio_to_telecmi(frame.audio)
        await self._client.publish_audio(telecmi_audio)

    def _convert_piopiy_audio_to_telecmi(self, piopiy_audio: bytes) -> rtc.AudioFrame:
...
class TelecmiTransport(BaseTransport):
    ...
    def __init__(
        ...
        # If any of these are missing, read from task-local ContextVars.
        if url is None:
            try:
                url = URL_CTX.get()
            except LookupError:
        ...
        if not (url and token and room_name):
            raise RuntimeError(
                "TelecmiTransport: missing url/token/room_name and no agent ContextVars set. "
                "Either pass them explicitly or ensure Agent set ContextVars before creating transport."
            )
