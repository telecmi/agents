...
"""Voxtral speech-to-text service implementation for piopiy pipeline."""
...
class VoxtralSTTService(STTService):
    """Voxtral speech-to-text service.

    Provides speech recognition using a Voxtral WebSocket server.
    Unlike streaming services, Voxtral processes complete audio segments,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            server_url: Voxtral WebSocket server URL.
        ...
        self.set_model_name("voxtral")

    @property
    def vad_enabled(self) -> bool:
        """VAD is handled by the pipeline, not Voxtral server."""
        return False
    ...
    async def set_model(self, model: str):
        """Set the model (not directly supported by Voxtral server)."""
        await super().set_model(model)
        logger.info(f"Model setting not supported by Voxtral server: {model}")
    ...
    async def start(self, frame: StartFrame):
        """Start the Voxtral STT service."""
        await super().start(frame)
        logger.debug(f"Starting Voxtral STT service with server: {self._server_url}")
        
        if self._persistent_connection:
            await self._connect()

    async def stop(self, frame: EndFrame):
        """Stop the Voxtral STT service."""
        await super().stop(frame)
        await self._finalize_audio()
        await self._disconnect()

    async def cancel(self, frame: CancelFrame):
        """Cancel the Voxtral STT service."""
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        """Process frames with Voxtral-specific handling."""
    ...
    async def _connect(self):
        """Establish WebSocket connection to Voxtral server."""
        ...
        async with self._connection_lock:
            ...
            try:
                logger.debug(f"Connecting to Voxtral server: {self._server_url}")
                ...
                logger.info("Connected to Voxtral server")
                ...
                logger.error(f"Failed to connect to Voxtral server: {e}")
    ...
    async def _disconnect(self):
        ...
        async with self._connection_lock:
            if self._websocket and not self._websocket.closed:
                logger.debug("Disconnecting from Voxtral server")
                try:
    ...
    async def _transcribe_accumulated_audio(self):
        ...
        try:
            ...
            # Generate request ID
            request_id = f"voxtral-{uuid.uuid4()}"
            ...
            if not self._persistent_connection:
                ...
                try:
                    await asyncio.wait_for(self._active_requests[request_id].wait(), timeout=30.0)
    ...
    async def _process_message(self, data: Dict[str, Any]):
        """Process a message from the Voxtral server."""
        ...
        if msg_type == "started":
            ...
            await self.push_error(ErrorFrame(f"Voxtral error: {error_msg}"))
            await self.stop_all_metrics()
